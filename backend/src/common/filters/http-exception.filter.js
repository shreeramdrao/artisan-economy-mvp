    ...
    var HttpExceptionFilter = _classThis = /** @class */ (function () {
        ...
        HttpExceptionFilter_1.prototype.catch = function (exception, host) {
            ...
            var error = typeof exceptionResponse === 'string'
                ...
                : exceptionResponse;
            this.logger.error("HTTP Exception: ".concat(request.method, " ").concat(request.url), JSON.stringify(error));
            response.status(status).json(__assign(__assign({}, error), { statusCode: status, timestamp: new Date().toISOString(), path: request.url }));
        };
